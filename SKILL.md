...
description: "Analyze an attached full-body person or character reference image and write a production-ready English prompt for Nano Banana Pro, FLUX, or Midjourney that creates a consistent vertical three-panel character sheet: a face-excluded front view framed from the base of the neck to the shoes, a complete head-to-toe back view, and a close-up face portrait. Use for requests such as 캐릭터 시트 프롬프트, 인물 설정 시트, 3면 캐릭터 레퍼런스, 의상 정면과 후면, 얼굴 클로즈업, or converting a character image into a reusable image-generation prompt."
---
...
3. Read [references/three-panel-spec.md](references/three-panel-spec.md) completely and apply every mandatory layout, analysis, preservation, and output rule.
...
- Show the middle panel from the top of the head to the soles, viewed from the back, without cropping.
- Use the right panel for the same character's complete facial identity and hair details.
...
- explicitly states the mandatory left, middle, and right panel framing;
